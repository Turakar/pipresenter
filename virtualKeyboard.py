"""
   ...
   Tilman Hoffbauer slightly tweaked this file to support passwords!
...
#        self.textbox = pygame.Surface((self.rect.width,self.keyH*2))
    ...
    def run(self, text='', password=False):

        self.text = text
        # create an input text box
        # create a text input box with room for 2 lines of text. leave room for the escape key
        self.input = TextInput(self.screen,self.text,self.x,self.y,self.textW,self.textH, password)
...
class TextInput():
    ...
    def __init__(self, screen, text, x, y, w, h, password=False):
        self.screen = screen
        self.text = text
...
# should be more general, but for now, just hack it for 2 lines
        ...
        if self.password:
            displayText = "*" * len(displayText)
        txt1 = displayText[:self.lineChars] # line 1
        txt2 = displayText[self.lineChars:] # line 2
